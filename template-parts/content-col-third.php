...
<article id="post-<?php the_ID(); ?>" <?php post_class('flex flex-wrap mb-8'); ?>>
    ...
    <div class="w-full">
        <header class="entry-header <?php echo ( is_singular() ) ? '' : 'ml-4'; ?>">
            ...
            <?php
            if ( is_singular() ) :
                the_title( '<h1 class="entry-title leading-none mb-4 text-4xl">', '</h1>' );
            else :
                the_title( '<h2 class="entry-title leading-none mb-4 text-4xl"><a href="' . esc_url( get_permalink() ) . '" class="text-gray-900" rel="bookmark">', '</a></h2>' );
            endif;
            ?>
        </header><!-- .entry-header -->
